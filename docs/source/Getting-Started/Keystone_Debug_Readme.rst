...
This guide walks through a successful Keystone QEMU debug session using
GDB, including PMP inspection using the ``pmp.py`` script.
...
This will start QEMU and pause waiting for a GDB connection on port
``:9822``.
...
+==============+=======+==========+======================+===========+
| **reg 0**    | NAPOT | ``0x18`` | ``0x8000             | Protected |
|              |       | (        | 0000 -> 0x80200000`` | memory    |
|              |       | RWX=000) | (2 MiB)              | for the   |
|              |       |          |                      | *         |
|              |       |          |                      | *security |
|              |       |          |                      | monitor** |
|              |       |          |                      | (SM).     |
...
|              |       |          |                      | access.   |
+--------------+-------+----------+----------------------+-----------+
| **reg 1**    | NAPOT | ``0x1f`` | ``0x8320             | EPM       |
|              |       | (        | 0000 -> 0x83400000`` | (Enclave  |
|              |       | RWX=111) | (2 MiB)              | Physical  |
|              |       |          |                      | Memory)   |
|              |       |          |                      | for an    |
|              |       |          |                      | active    |
|              |       |          |                      | enclave.  |
...
|              |       |          |                      | /execute. |
+--------------+-------+----------+----------------------+-----------+
| **reg 2**    | NAPOT | ``0x1f`` | ``0x0 -> 0x8`` (8    | Guard     |
|              |       | (        | bytes)               | region to |
|              |       | RWX=111) |                      | catch     |
|              |       |          |                      | nul       |
|              |       |          |                      | l-pointer |
...
|              |       |          |                      | region.   |
+--------------+-------+----------+----------------------+-----------+
| **reg 7**    | NAPOT | ``0x1f`` | ``0x831c             | UTM       |
|              |       | (        | 0000 -> 0x83200000`` | (         |
|              |       | RWX=111) | (256 KiB)            | Untrusted |
|              |       |          |                      | Memory)   |
|              |       |          |                      | shared    |
|              |       |          |                      | between   |
|              |       |          |                      | host and  |
...
-  Each ``addr`` is decoded to show the base and top (exclusive) of the
   region it protects.
...
-  Use ``pmp-clear`` if you want to temporarily disable all PMP regions
   during debugging.

This setup enables effective Keystone debugging and enclave memory
protection inspection in QEMU using GDB, but however it stills on
progress..
